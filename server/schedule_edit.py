...
class ScheduleEdit:
    ...
    Typical usage:
        edit = ScheduleEdit(user, sessions_to_add=[s1], sessions_to_cancel=[s2])
    ...
    def __init__(self, user: User, sessions_to_add=None, sessions_to_cancel=None):
        self.user = user
    ...
    def filter_conflicting(self) -> list:
        ...
        for room in set(s.room for s in self.sessions_to_add):
            existing = [si.session for si in Schedule(room).schedule
    ...
    def book_sessions(self) -> list:
        ...
        for s in self.sessions_to_add:
            ...
            if self.user.credits < cost:
                continue
            if Schedule(s.room).add_session(s, self.user.id):
...
if __name__ == '__main__':
    ...
    # ── Redirect all file storage to temp dir ─────────────────────────────────
    _tmp_dir = tempfile.mkdtemp()
    from schedule import Schedule as _Sch
    from user import UserManager as _UM
    _orig_sch = _Sch._FILE_PATH
    _orig_usr = _UM.FILE_PATH
    _Sch._FILE_PATH = os.path.join(_tmp_dir, 'schedule.csv')
    _UM.FILE_PATH   = os.path.join(_tmp_dir, 'user.csv')
    _Sch._SESSIONS.clear()
    _Sch.LAST_UPDATE = 0
    _UM._USERS.clear()
    _UM._LAST_UPDATE = 0
    ...
    # ── Test 1: basic book ────────────────────────────────────────────────────
    s1 = Session(t1, 60, 0)
    edit = ScheduleEdit(alice, sessions_to_add=[s1])
    ...
    edit2 = ScheduleEdit(alice, sessions_to_add=[s2])
    ...
    edit3 = ScheduleEdit(alice, sessions_to_add=[s3])
    edit3.filter_ended()
    ...
    edit4 = ScheduleEdit(alice, sessions_to_cancel=[s1])
    ...
    Schedule(0).add_session(s5, alice.id)

    edit5 = ScheduleEdit(alice, sessions_to_add=[s6], sessions_to_cancel=[s5])
    ...
    s7 = Session(t3, 30, 1)
    edit6 = ScheduleEdit(alice, sessions_to_add=[s7], sessions_to_cancel=[s5])
