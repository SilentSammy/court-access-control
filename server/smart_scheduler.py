...
from server.schedule import ScheduleDisplayer, ScheduleItem, RoomSchedule
import re
...
class SmartScheduler:
    ...
    def __init__(self, user: User, schedules: list[RoomSchedule]):
    ...
    def get_schedule_for_room(self, room_id: int):
        """Find the RoomSchedule that contains the given room_id."""
        for schedule in self.schedules:
            if room_id in schedule.room_ids:
                return schedule
        return None
    ...
    def get_prompt(self):
        ...
        # Get all available room types from the schedules
        room_types = [schedule.name for schedule in self.schedules]
        room_types_str = ", ".join([f'"{rt}"' for rt in room_types])
        ...
        user_sessions = [
            {
                ...
                'room_type': self.get_schedule_for_room(session.room).name if self.get_schedule_for_room(session.room) else "Unknown",
        ...
        context = (
            f"Available room types: {room_types_str}\n"
        ...
        prompt = (
            f'You work for a sports center with multiple types of facilities: {room_types_str}.\n'
            ...
            '\n'
            'IMPORTANT: Users will specify which type of facility they want (e.g., "Squash", "Tennis"). '
            'You should capture this in the "room_type" field using one of the available room types listed above. '
            'The system will automatically assign a specific room from that type. '
            'DO NOT use or specify room_id - that is handled automatically.\n'
            '\n'
            ...
            '- room_type (string): the facility type name, must be one of: ' + room_types_str + '.\n'
            ...
            '\n'
            'EXAMPLE 1:\n'
            f'User input: I want to play Squash this weekend at 5pm, for 1h.\n'
            'Output:\n'
            f'I will add a session for Saturday at 5pm for 1 hour. The user wants to play Squash, so I will set room_type to "Squash Courts".\n'
            '{"action": "add", "room_type": "Squash Courts", "start_date": "' + saturday + '", "start_time": "17:00", "duration": 60}\n'
            f'I will also add a session for Sunday at 5pm for 1 hour.\n'
            '{"action": "add", "room_type": "Squash Courts", "start_date": "' + sunday + '", "start_time": "17:00", "duration": 60}\n'
            '\n'
            'EXAMPLE 2:\n'
            'Suppose the user has the following sessions:\n'
            '{"session_id": 0, "room_id": 1, "room_type": "Squash Courts", "start_date": "' + saturday + '", "start_time": "10:00", "duration": 60}' + '\n'
            '{"session_id": 1, "room_id": 1, "room_type": "Squash Courts", "start_date": "' + saturday + '", "start_time": "12:00", "duration": 60}' + '\n'
            '{"session_id": 2, "room_id": 2, "room_type": "Squash Courts", "start_date": "' + sunday + '", "start_time": "14:00", "duration": 60}' + '\n'
            f'User input: I want to cancel all my Saturday sessions.\n'
            'Output:\n'
            f'I will cancel the sessions on Saturday.\n'
            '{"action": "remove", "session_id": 0}\n'
            '{"action": "remove", "session_id": 1}\n'
            '\n'
            'EXAMPLE 3:\n'
            'Suppose the user has the following sessions:\n'
            '{"session_id": 0, "room_id": 1, "room_type": "Squash Courts", "start_date": "' + saturday + '", "start_time": "10:00", "duration": 90}' + '\n'
            '{"session_id": 1, "room_id": 2, "room_type": "Squash Courts", "start_date": "' + sunday + '", "start_time": "12:00", "duration": 120}' + '\n'
            f'User input: Move my upcoming session to 3pm.\n'
            'Output:\n'
            ...
            '{"action": "add", "room_type": "Squash Courts", "start_date": "' + saturday + '", "start_time": "15:00", "duration": 90}\n'
            '\n'
        )
        return prompt
    ...
    def process_user_message(self, message):
        ...
        # Generate a response using a GPT language model (OpenAI v2 API)
    ...
    def try_get_session(self, sess_dict:dict, exclude_sessions=None):
        """Attempts to convert a session dictionary to a session object.
        
        Args:
            sess_dict: Dictionary with session info (action, room_type, start_date, etc.)
            exclude_sessions: Optional list of Session objects to treat as unavailable
                             when finding available rooms (used for batch booking)
        """
        ...
        try:
            ...
            if sess_dict["action"] == "add":
                ...
                room_type = sess_dict.get("room_type", "")

                # Find the schedule that matches the room type
                target_schedule = None
                for schedule in self.schedules:
                    if schedule.name == room_type:
                        target_schedule = schedule
                        break
                
                if target_schedule is None:
                    # Invalid room type specified
                    return None
                ...
                # Find an available room from the specified room type
                # Check each room, excluding the ones already assigned in this batch
                sess = None
                for room_id in target_schedule.room_ids:
                    candidate = Session(start_timestamp, duration, room_id)
                    
                    # Check if this candidate conflicts with any excluded sessions
                    conflicts_with_excluded = any(
                        candidate.room == exc.room and candidate.conflicts_with(exc)
                        for exc in exclude_sessions
                    )
                    
                    if not conflicts_with_excluded and target_schedule.is_available(candidate):
                        sess = candidate
                        break
                
                if sess is None:
                    # No available room at this time
                    return None
                
                # check if the session has ended
                if sess.end < datetime.now().timestamp():
                    return None
    ...
    def try_get_sessions(self, sess_dicts):
        ...
        When converting multiple sessions, passes previously converted sessions to avoid
        assigning overlapping time slots to the same room (enables batch booking).
        """
        sessions = []
        for sess_dict in sess_dicts:
            # Pass already-converted sessions so we don't double-book the same room
            sess = self.try_get_session(sess_dict, exclude_sessions=sessions)
            if sess is not None:
...
def full_test():
    ...
    while True:
        ...
        for room in set(session.room for session in sessions_to_add):
            ...
            schedule = [si.session for si in self.room_schedule.schedule if si.session.room == room]
        ...
        for room_id, sessions in sessions_by_room.items():
            # Find the schedule that contains this room
            room_schedule = smart_scheduler.get_schedule_for_room(room_id)
            if room_schedule is None:
                continue
            
            displayer = ScheduleDisplayer(room_schedule)
...
if __name__ == "__main__":
    # Import schedule_config from parent directory
    from schedule_config import global_schedule, schedules
    
    # Create a user and SmartScheduler object
    test_user = User("50766180742", global_schedule)
    smart_scheduler = SmartScheduler(test_user, schedules)
    
    print("=== Smart Scheduler Test ===")
    print(f"User: {test_user.id}")
    print(f"Balance: {test_user.credits} credits")
    print(f"Available room types: {', '.join([s.name for s in schedules])}")
    print(f"Current sessions: {len(test_user.sessions)}\n")
